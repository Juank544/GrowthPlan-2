...
public class CoachController {
    ...
    public ResponseEntity<List<CoachDTO>> findAllCoaches() {
        List<CoachDTO> coaches = coachService.findAll().stream().map(coachMapper::toDTO).toList();
    ...
    public ResponseEntity<CoachDTO> findCoachById(@PathVariable UUID id) {
        ...
        return optionalCoach.map(coach -> new ResponseEntity<>(coachMapper.toDTO(coach), HttpStatus.OK))
